...
namespace Stocks
{
    public sealed class Server
    {
        ...
        private static readonly string Password = "Hd3sfg4PW94";//"GFKZNR"; //"TLQ23W";
        private readonly string Portfolio = "BP16181-RF-01";//"ST105396-RF-01"; //"BP16181-RF-01";
        private const int UpdatePositionInterval = 5 * 60 * 1000;
        ...
        private System.Timers.Timer CheckEndOfDayTimer;
        private System.Timers.Timer PingTimer;
        private const int CheckEndOfDayInterval = 1 * 37 * 1000;
        private const int PingTimerInterval = 2 * 60 * 1000;
        ...
        private Server(string login, string password, MainForm form)
        {
            ...
            WasConnected = false;
            dbWriter.InsertSts(ServerTime.GetRealTime(), "Stocks", "Working");
        }
        ...
        public bool IsConnected
        {
            get
            {
                ...
                {
                    throw new SmartException(ExceptionImportanceLevel.MEDIUM, "IsConnected", "Server", e.Message);
                }
            }
        }
        ...
        public static StBarInterval BarLenCast(int minutes)
        {
            switch (minutes)
            {
                ...
                default:
                    throw new SmartException(ExceptionImportanceLevel.MEDIUM, "BarLenCast", "Server", "minutes = " + minutes + ": not found");
            }
        }
        ...
        private void ConnectStockServer()
        {
            if (!IsConnected)
            {
                try
                {
                    ...
                    if (WasConnected)
                    {
                        EmailSender.SendEmail("Stocks Stopped", "Robot Stopped because the connection was lost. Will be reconnected by Monitor");
                        Environment.Exit(0);
                    } else {
                        WasConnected = true;
                    }
        ...
        public bool IsNowNightTime()
        {
            DateTime dTime = ServerTime.GetRealTime();
            return dTime.Hour <= 8 && dTime.Hour >= 0;
        }
        ...
        public void StartWork()
        {
            if (IsNowNightTime())
            {
                WriteToLog("Night Time. Program Interrupted.");
                Environment.Exit(0);
            }
            InitializeServer();
            ConnectStockServer();
        ...
        private void InitializeUpdatePositionTimer()
        {
        ...
        private void InitializeCheckEndOfDayTimer()
        {
            if (CheckEndOfDayTimer != null)
                CheckEndOfDayTimer.Stop();
            CheckEndOfDayTimer = new System.Timers.Timer(CheckEndOfDayInterval);
            CheckEndOfDayTimer.AutoReset = true;
            CheckEndOfDayTimer.Elapsed += CheckEndOfDayTimer_Elapsed;
            CheckEndOfDayTimer.Start();
        }
        private void InitializePingTimer()
        {
            if (PingTimer != null)
                PingTimer.Stop();
            PingTimer = new System.Timers.Timer(PingTimerInterval);
        ...
        private void CheckEndOfDayTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            CheckEndOfDay();
        }
        ...
        public void PlaceOrder(string symbol, ActionEnum action, OrderTypeEnum type, double price, double volume, double stopPrice, int cookie)
        {
            try
            {
                if (IsConnected)
                {
                    ...
                    StOrder_Validity validity = (type == OrderTypeEnum.STOP ? StOrder_Validity.StOrder_Validity_Day : StOrder_Validity.StOrder_Validity_Gtc);
                    SmartComServer.PlaceOrder(Portfolio, symbol, action == ActionEnum.BUY ? StOrder_Action.StOrder_Action_Buy : StOrder_Action.StOrder_Action_Sell
                        , type == OrderTypeEnum.LIMIT ? StOrder_Type.StOrder_Type_Limit : StOrder_Type.StOrder_Type_StopLimit, validity
