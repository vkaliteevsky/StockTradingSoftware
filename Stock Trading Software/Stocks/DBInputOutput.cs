...
namespace Stocks
{
    public class DBInputOutput
    {
        ...
        public class DBWriter
        {
            ...
            public void InsertTrade(string symbol, DateTime dateTime, double price, double volume)
            {
                try
                {
                    lock (LockObject)
                    {
                        MySqlCommand insertCommand = DatabaseConnection.CreateCommand();
                        insertCommand.CommandText = "insert into trades(symbol, dtime, price, volume) values (?tSymbol, ?tDateTime, ?tPrice, ?tVolume)";
                        insertCommand.Parameters.AddWithValue("?tSymbol", symbol);
                        insertCommand.Parameters.AddWithValue("?tDateTime", dateTime.ToString(DateTimeFormat));
                        insertCommand.Parameters.AddWithValue("?tPrice", price);
                        insertCommand.Parameters.AddWithValue("?tVolume", volume);
                        insertCommand.ExecuteNonQuery();
            ...
            public void TruncateTrades(DateTime beforeTime)
            {
                try
                {
                    lock (LockObject)
                    {
                        MySqlCommand insertCommand = DatabaseConnection.CreateCommand();
                        insertCommand.CommandText = "delete from trades where dtime < '?dBeforeTime'";
                        insertCommand.Parameters.AddWithValue("?dBeforeTime", beforeTime.ToString(DateTimeFormat));
                        insertCommand.ExecuteNonQuery();
                    }
                } catch (Exception e)
                {
            ...
            public void TruncateOrderBook(DateTime beforeTime)
            {
                try
                {
                    lock (LockObject)
                    {
                        MySqlCommand insertCommand = DatabaseConnection.CreateCommand();
                        insertCommand.CommandText = "delete from orderbook where dtime < ?dBeforeTime";
                        insertCommand.Parameters.AddWithValue("?dBeforeTime", beforeTime.ToString(DateTimeFormat));
            ...
            public void TruncateGeneral(DateTime beforeTime)
            {
                try
                {
                    lock (LockObject)
                    {
                        MySqlCommand insertCommand = DatabaseConnection.CreateCommand();
                        insertCommand.CommandText = "delete from general where dtime < ?dBeforeTime";
                        insertCommand.Parameters.AddWithValue("?dBeforeTime", beforeTime.ToString(DateTimeFormat));
                        insertCommand.ExecuteNonQueryAsync();
                    }
                }
            ...
            public void InsertSts(DateTime dTime, string program, string status)
            {
                try
                {
                    lock (LockObject)
                    {
                        MySqlCommand insertCommand = DatabaseConnection.CreateCommand();
                        insertCommand.CommandText = "insert into sts (dtime, prog, status) values (?dTime, ?dProg, ?dStatus)";
                        insertCommand.Parameters.AddWithValue("?dTime", dTime.ToString(DateTimeFormat));
                        insertCommand.Parameters.AddWithValue("?dProg", program);
                        insertCommand.Parameters.AddWithValue("?dStatus", status);
                        insertCommand.ExecuteNonQuery();
                    }
                }
                catch
                {
                    throw new SmartException(ExceptionImportanceLevel.HIGH, "InsertSts", "DBInputOutput", "Unexpected Exception");
                }
            }
            ...
            public void InsertOrderBook(DateTime dTime, string symbol, SmartCOM3Lib.StBarInterval interval,
                double open, double high, double low, double close, int volume)
            {
                try
                {
                    ...
                    {
                        MySqlCommand insertCommand = DatabaseConnection.CreateCommand();
                        insertCommand.CommandText = "insert into orderbook (symbol, dtime, typemins, open, high, low, close, volume) values" +
                            "(?dSymbol, ?dTime, ?dTypeMins, ?dOpen, ?dHigh, ?dLow, ?dClose, ?dVolume)";
                        insertCommand.Parameters.AddWithValue("?dSymbol", symbol);
                        insertCommand.Parameters.AddWithValue("?dTime", dTime.ToString(DateTimeFormat));
                        insertCommand.Parameters.AddWithValue("?dTypeMins", Server.BarLenCast(interval));
                        insertCommand.Parameters.AddWithValue("?dOpen", open);
                        insertCommand.Parameters.AddWithValue("?dHigh", high);
                        insertCommand.Parameters.AddWithValue("?dLow", low);
                        insertCommand.Parameters.AddWithValue("?dClose", close);
                        insertCommand.Parameters.AddWithValue("?dVolume", volume);
                        insertCommand.ExecuteNonQuery();
                    }
                } catch (Exception e)
        ...
        public class DBReader
        {
            ...
            public List<Bar> SelectBars(string symbol, int amount)
            {
                try
                {
                    lock (LockObject)
                    {
                        ...
                        MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                        selectCommand.CommandText = "select * from orderbook where symbol = ?dSymbol order by id desc limit ?dAmount";
                        selectCommand.Parameters.AddWithValue("?dSymbol", symbol);
                        selectCommand.Parameters.AddWithValue("?dAmount", amount);
                        using (MySqlDataReader reader = selectCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                int volume = reader.GetInt32("volume");
                                bars.Add(new Bar(open, close, low, high, volume, dTime));
                            }
                            reader.Close();
                        }
                        bars.Reverse();
            ...
            public Bar SelectLastPrice(string symbol)
            {
                try
                {
                    lock (LockObject)
                    {
                        Bar bar = new Bar();
                        MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                        selectCommand.CommandText = "select * from trades where symbol = ?dSymbol order by id desc limit 1";
                        selectCommand.Parameters.AddWithValue("?dSymbol", symbol);
                        using (MySqlDataReader reader = selectCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                DateTime dTime = reader.GetDateTime("dtime");
                                double close = reader.GetDouble("price");
                                int volume = reader.GetInt32("volume");
                                bar = new Bar(close, close, close, close, volume, dTime);
                            }
                            reader.Close();
                        }
                        return bar;
            ...
            public List<Tuple<DateTime, string, string>> SelectSts(string prog, int amount, string status = "")
            {
                try
                {
                    lock (LockObject)
                    {
                        List<Tuple<DateTime, string, string>> res = new List<Tuple<DateTime, string, string>>();
                        MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                        if (status.Equals(""))
                        {
                            selectCommand.CommandText = "select * from sts where prog = ?dProg order by id desc limit ?dAmount";
                        } else
                        {
                            selectCommand.CommandText = "select * from sts where prog = ?dProg and status = ?dStatus order by id desc limit ?dAmount";
                            selectCommand.Parameters.AddWithValue("?dStatus", status);
                        }
                        selectCommand.Parameters.AddWithValue("?dProg", prog);
                        selectCommand.Parameters.AddWithValue("?dAmount", amount);
                        using (MySqlDataReader reader = selectCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                DateTime dTime = reader.GetDateTime("dtime");
                                string program = reader.GetString("prog");
                                string stat = reader.GetString("status");
                                res.Add(new Tuple<DateTime, string, string>(dTime, program, stat));
                            }
                            reader.Close();
                        }
                        return res;
            ...
            public List<Tuple<DateTime, string, string>> SelectGeneral(string ticker, int amount)
            {
                try
                {
                    lock (LockObject)
                    {
                        List<Tuple<DateTime, string, string>> res = new List<Tuple<DateTime, string, string>>();
                        MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                        selectCommand.CommandText = "select * from general where ticker = ?dSymbol order by id desc limit ?dAmount";
                        selectCommand.Parameters.AddWithValue("?dSymbol", ticker);
                        selectCommand.Parameters.AddWithValue("?dAmount", amount);
                        using (MySqlDataReader reader = selectCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                string tkr = reader.GetString("ticker");
                                res.Add(new Tuple<DateTime, string, string>(dTime, comment, tkr));
                            }
                            reader.Close();
                        }
                        return res;
            ...
            public Tuple<List<DateTime>, List<double>> ReadInfo(string symbol, string propName, DateTime fromDate)
            {
                try
                {
                    Tuple<List<DateTime>, List<double>> res = new Tuple<List<DateTime>, List<double>>(new List<DateTime>(), new List<double>());
                    MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                    selectCommand.CommandText = "select * from info where symbol = ?dSymbol and prop_name = ?dPropName and date >= ?dFromDate order by id desc";
                    selectCommand.Parameters.AddWithValue("?dSymbol", symbol);
                    selectCommand.Parameters.AddWithValue("?dPropName", propName);
                    selectCommand.Parameters.AddWithValue("?dFromDate", fromDate.ToString("yyy-MM-dd"));
                    using (MySqlDataReader reader = selectCommand.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            res.Item1.Add(reader.GetDateTime("date"));
                            res.Item2.Add(reader.GetDouble("val"));
                        }
                        reader.Close();
                    }
                    return res;
            ...
            public Tuple<List<DateTime>, List<double>> ReadInfo(string propName, DateTime fromDate)
            {
                try
                {
                    Tuple<List<DateTime>, List<double>> res = new Tuple<List<DateTime>, List<double>>(new List<DateTime>(), new List<double>());
                    MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                    selectCommand.CommandText = "select * from info where prop_name = ?dPropName and date >= ?dFromDate order by id desc";
                    //selectCommand.Parameters.AddWithValue("?dSymbol", symbol);
                    selectCommand.Parameters.AddWithValue("?dPropName", propName);
                    selectCommand.Parameters.AddWithValue("?dFromDate", fromDate.ToString("yyy-MM-dd"));
                    using (MySqlDataReader reader = selectCommand.ExecuteReader())
            ...
            public List<DateTime> ReadGeneral(DateTime fromDate)
            {
                List<DateTime> res = new List<DateTime>();
                try
                {
                    MySqlCommand selectCommand = DatabaseConnection.CreateCommand();
                    selectCommand.CommandText = "select dtime from general where dtime >= ?dFromDate and ticker <> 'Listener'";
                    selectCommand.Parameters.AddWithValue("?dFromDate", fromDate.ToString(DateTimeFormat));
                    using (MySqlDataReader reader = selectCommand.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            res.Add(reader.GetDateTime("dtime"));
                        }
                        reader.Close();
                    }
                    return res;
